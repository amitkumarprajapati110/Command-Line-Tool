...
time ./build/ls_single -R "$test_space/$target"  > /dev/null 2>&1
echo "Running test for 'cp'"
time ./build/cp_single -R "$test_space/$target" "$test_space/dummy"  > /dev/null 2>&1
echo "Running test for 'rm'"
time ./build/rm_single -R "$test_space/$target"  > /dev/null 2>&1
mkdir -p "$test_space/$target" # Ensure destination exists
echo "Running test for 'mv'"
time ./build/mv_single -R "$test_space/dummy" "$test_space/$target"  > /dev/null 2>&1
